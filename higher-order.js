...
let evenNumbers = mixedNumbers.filter(function(n) {
  if (n % 2 === 0) {
    return n;
  } else {
    return;
...
//Code Here
let totalPopulation = populations.reduce(function(total, sum) {
  return total + sum;
});
...
//Code Here
let myStrongest = monstersInYourPocket.filter(function(n) {
  if (n.CP > 200) {
    return n;
  }
});
...
let orderTotals = orders.map(function(n) {
  return n.price + n.price * n.tax;
});
...
let bobsTotal = purchases.reduce(function(total, n) {
  if (n.owner === "Bob") {
